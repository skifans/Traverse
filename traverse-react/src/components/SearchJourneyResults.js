...
export default class SearchJourneyResults extends Component{
  ...
  componentWillMount() {
    ...
    .then(data => console.log(this.state.data[0].routes))
    ...
    if (this.state.loading) {
      ...
      return (
        <main>
          ...
          <JourneyDetails journeyDetails={this.state.data[0].routes}/>
          <div id="results">
            <h3>Recommended journeys</h3>
            <div className="entries">
              {Array.from(Array(this.state.data[0].routes.routes.length)).map((x, index) => <Journey journey={this.state.data[0].routes} i={index} />)}
            </div>
          </div>
        </div>
      </main>
